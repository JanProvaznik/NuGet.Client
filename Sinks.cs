...
internal static class SinkRules
{
    ...
    public static bool TryClassifyEvent(IEventSymbol e, string action, out SinkCategory category, out string api)
    {
        category = default;
        api = "";

        // Only a subscription (+=) accumulates handlers across reused-process builds. A raise (Invoke) or an
        // unsubscription (-=) does not add un-paired global state, so it is not a leak sink. (-= is still surfaced
        // so reviewers can see +=/-= pairing.)
        if (action == "raise")
        {
            return false;
        }
        ...
        if (type == "System.AppDomain")
        {
            ...
            api = "System.AppDomain." + e.Name + " (" + action + ")";
            return true;
        }
        if (e.IsStatic)
        {
            category = SinkCategory.StaticEventSubscription;
            api = type + "." + e.Name + " (" + action + ")";
            return true;
        }
        return false;
    }
