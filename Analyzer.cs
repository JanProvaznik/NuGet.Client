...
internal sealed class Analyzer
{
        ...
        while (stack.Count > 0)
        {
            ...
            switch (op)
            {
                ...
                case IEventReferenceOperation er:
                    HandleEvent(er.Event, er, methodId, "raise");
                    break;
                case IEventAssignmentOperation ea when ea.EventReference is IEventReferenceOperation er2:
                    HandleEvent(er2.Event, ea, methodId, ea.Adds ? "subscribe" : "unsubscribe");
                    break;
    ...
    private void HandleEvent(IEventSymbol e, IOperation op, string methodId, string action)
    {
        if (SinkRules.TryClassifyEvent(e, action, out var cat, out var api))
        {
    ...
    private void RecordStaticField(IFieldSymbol f, string hostId, bool read, bool written, bool initialized)
    {
        ...
        if (!_static.TryGetValue(key, out var sf))
        {
            sf = new StaticStateFinding
            {
                ...
                TypeShape = ClassifyShape(f.Type),
                File = LocFile(f),
                Line = LocLine(f),
            };
            _static[key] = sf;
    ...
    private void RecordStaticProperty(IPropertySymbol p, string hostId, bool read, bool written, bool initialized = false)
    {
        ...
        if (!_static.TryGetValue(key, out var sf))
        {
            sf = new StaticStateFinding
            {
                ...
                TypeShape = ClassifyShape(p.Type),
                File = LocFile(p),
                Line = LocLine(p),
            };
            _static[key] = sf;
    ...
    // Coarse "shape" of a static member's declared type, used by the resetability annotator to decide whether
    // the member can hold per-build/per-environment/per-machine state (a real leak) versus being immutable, a
    // pure input-keyed cache, or a live OS resource. Conservative: anything unrecognized falls through to "other"
    // and is surfaced for manual review.
    private static string ClassifyShape(ITypeSymbol t)
    {
        if (!IsMutableRef(t)) { return "immutable"; }

        // Unwrap Lazy<T>/Nullable<T> to look at the payload.
        ITypeSymbol core = t;
        if (core is INamedTypeSymbol named && named.IsGenericType)
        {
            string od0 = named.OriginalDefinition.ToDisplayString();
            if (od0 is "System.Lazy<T>" or "System.Nullable<T>" or "System.Threading.ThreadLocal<T>" or "System.Threading.AsyncLocal<T>")
            {
                core = named.TypeArguments[0];
            }
        }

        if (HoldsLiveResource(core)) { return "liveresource"; }

        string od = core.OriginalDefinition.ToDisplayString();
        string name = core.Name;

        if (od.StartsWith("System.Collections.Immutable.", StringComparison.Ordinal)
            || od.StartsWith("System.Collections.Frozen.", StringComparison.Ordinal)
            || name.StartsWith("ReadOnly", StringComparison.Ordinal))
        {
            return "immutable-collection";
        }

        if (Implements(core, "System.Collections.Generic.IEqualityComparer<T>")
            || Implements(core, "System.Collections.Generic.IComparer<T>")
            || name.EndsWith("Comparer", StringComparison.Ordinal))
        {
            return "comparer";
        }

        if (name is "ResourceManager" or "Encoding" or "Regex" or "JsonSerializerOptions"
            or "RecyclableMemoryStreamManager" || name.EndsWith("JsonContext", StringComparison.Ordinal)
            || name.EndsWith("SerializerContext", StringComparison.Ordinal))
        {
            return "stateless-helper";
        }

        // Object pools / weak tables: memoization-style holders keyed on inputs.
        if (name.Contains("ObjectPool") || name.Contains("Pool") || od.StartsWith("System.Runtime.CompilerServices.ConditionalWeakTable", StringComparison.Ordinal))
        {
            return "pool";
        }

        // Mutable collections used as caches/tables.
        if (od.StartsWith("System.Collections.Concurrent.", StringComparison.Ordinal)) { return "concurrent-collection"; }
        if (od.StartsWith("System.Collections.Generic.Dictionary", StringComparison.Ordinal)
            || od.StartsWith("System.Collections.Generic.HashSet", StringComparison.Ordinal)
            || od.StartsWith("System.Collections.Generic.List", StringComparison.Ordinal)
            || od.StartsWith("System.Collections.Generic.SortedDictionary", StringComparison.Ordinal)
            || od.StartsWith("System.Collections.Generic.SortedSet", StringComparison.Ordinal)
            || t.TypeKind == TypeKind.Array)
        {
            return "collection";
        }

        if (Implements(core, "System.IDisposable")) { return "disposable"; }

        return "other";
    }

    private static bool HoldsLiveResource(ITypeSymbol t)
    {
        static bool IsResource(string n) => n is
            "System.Threading.Timer" or "System.Timers.Timer"
            or "System.Diagnostics.Process" or "System.Diagnostics.ProcessStartInfo"
            or "System.Net.Sockets.Socket" or "System.Net.Http.HttpClient" or "System.Net.Http.HttpMessageHandler"
            or "System.Net.Http.HttpClientHandler" or "System.Net.Http.SocketsHttpHandler"
            or "System.Threading.SemaphoreSlim" or "System.Threading.Semaphore" or "System.Threading.Mutex"
            or "System.Threading.CancellationTokenSource" or "System.IO.FileStream" or "System.IO.Stream";

        if (IsResource(t.OriginalDefinition.ToDisplayString())) { return true; }
        if (t is INamedTypeSymbol n && n.IsGenericType)
        {
            foreach (var arg in n.TypeArguments)
            {
                if (IsResource(arg.OriginalDefinition.ToDisplayString())) { return true; }
            }
        }
        return false;
    }

    private static bool Implements(ITypeSymbol t, string ifaceOriginalDef)
    {
        if (t.OriginalDefinition.ToDisplayString() == ifaceOriginalDef) { return true; }
        foreach (var i in t.AllInterfaces)
        {
            if (i.OriginalDefinition.ToDisplayString() == ifaceOriginalDef) { return true; }
        }
        return false;
    }

    private static string LocFile(ISymbol s)
    {
        var loc = s.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax().GetLocation().GetLineSpan();
        if (loc is null) { return ""; }
        const string root = @"Q:\nuget.client\";
        string p = loc.Value.Path;
        if (p.StartsWith(root, StringComparison.OrdinalIgnoreCase)) { p = p.Substring(root.Length); }
        return p.Replace('\\', '/');
    }

    private static int LocLine(ISymbol s)
    {
        var loc = s.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax().GetLocation().GetLineSpan();
        return loc is null ? 0 : loc.Value.StartLinePosition.Line + 1;
    }
